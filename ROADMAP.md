...
| D3  | `sync-order-to-pennylane` workflow — orchestrates C1 → D1 → D2 → persist to `InvoiceSync`                          | ✅     | [doc](docs/sync-order-workflow.md). `pickCapturedPayment` helper; `action: "created" \| "idempotent" \| "already-synced"`; 15 new tests.              |
| D4  | Subscriber on `payment.captured` — invokes D3                                                                      | ✅     | [doc](docs/payment-captured-subscriber.md). Auto-sync default-on; opt-out via `autoSyncOnCapture: false`. Orphan payments skip at DEBUG. 9 new tests. |
